...
/* Maximum number of data words that any test function accesses */
#define MAX_WORDS       32
...
static int worker_thread(void *arg)
{
	...
	uint32_t *c = b;
	uint32_t num_func = test_options->num_func;
	...
	global->stat[thr].loops     = rounds * num_func;
...
static void print_stat(test_global_t *global)
{
	...
	for (i = 1; i < num_cpu + 1; i++) {
		nsec   = global->stat[i].tot_nsec;
		loops  = global->stat[i].loops;
	...
	printf("  ave loops per sec: %.2f k/sec\n", (sum_loops / (sum_nsec / giga)) / 1000.0);
	printf("\n");
}
